...
author: "Gillam, Gillam & Snader 1, NCCI 1, Venter"
...
[Dorweiler](#Dorweiler) test

[Different iterations](#NCCI-history) of the NCCI plans

* All the changes and purpose
* Assumptions on the $B$ and $K$

[Purpose](#exp-rating-purpose) of experience rating
...
CALCULATING G, SRP, SACC

* Formula calc
* The SACC data should be trended from the midpoint of the last three years of unit plan data to the midpoint of the experience period
* G is calculated using losses used in experience rating, so technically there's limit now after 1998 changes
...
EVALUATING EXPERIENCE RATING PLANS

1. Do this for current and proposed group
2. Sort by mod in increasing order
3. Group into 5 quintiles (Or whatever the questions tell you like low and high)
4. Get standard and manual LR for each group
5. Do $\dfrac{\sigma^2(Standard \:  LR)}{\sigma^2(Manual \: LR)}$
6. Lower statistic is better (Can tell something by just eye balling the spreads of manual and standard)
...
1961 PROGRAM: $M,\: A_P,\: W,\: Z$

* The formula for $W$ is a bit different
...
STANDARD PREMIUM & MOD FACTOR,EXPERIENCE PERIOD, CAPPED LOSSES

* Just lookups

MULTI STATE MOD

* Note that you get the state B and W by the total E and then weight by state E

INTRA STATE MOD SINGLE STATE ALL OTHER STATES

* Intrastate you have to adjust for the fact that the total $\neq$ sum of all of it's parts
* $\dfrac{Mod_{all} \times E_{all}}{Mod_{St} \times E_{St} + Mod_{AllxSt} \times E_{AllxSt}}$
* Get the state mod and times the factor
...
<span style="color:green">2014 Q9</span> NCCI uses loss only (no ALAE)
...
**Loss free mod formula:**

$\begin{array}{lccc}
  Mod &= &\frac{1}{E}[Z_p A_p + Z_e A_e] &+ &\frac{1}{E}[ (1-Z_p)E_p + (1-Z_e)E_e] \\
  Mod &= &\frac{1}{E}[Z_p A_p + Z_e A_e] &+ &\text{loss-free mod} \\
\end{array}$
...
* Weight: $W = \dfrac{E + K_p}{E + K_e}$
    * $0 < W < 1$
    * $W$ never $\uparrow$ given $\downarrow$ $E$
    * For the 1961 NCCI it's $W = \dfrac{\text{Expected Loss} - \text{Excess Cred. Threshold}}{\text{Full Cred. Threshold} - \text{Excess Cred. Threshold}}$
...
* Small risk with credit mod doesn't get enough premium <span style="color:green">2014 Q11</span>
...
1. $\downarrow$ $Z$ on Med-only by about 70%
    ...
    * In practice it's split into primary and excess first then only use 30% <span style="color:green">2002 Q33</span>
...
#### Calculation Based Process Map

![alt text](figures/Exam 8 B8 - 15 - 6.png)

#### Lookup Based Process Map

![alt text](figures/Exam 8 B8 - 15 - 8.png)
...
* $t$ = trend period = midpoint of mod experience period to average accident date for prospective policy
...
Gillam, written in 1992, states there is no per-claim limit applied to losses in the G calculation. The NCCI manual written after 1998 states (rule D.2) that G is calculated using losses used in experience rating; suggesting limitations do apply (e.g. include 30% of medical only).
...
$> +20%$ change will be investigated; Similar to SRP, can't decrease

***

#### Ballast (B),  Weighting Value (W) and C
...
Application is different depending whether it exceed the multi and single limit

* There are single and multiple claims limit

* The dollar amounts limit is under ther weighting values table
...
Ownership changes and combination of entities when they share common majority ownership
