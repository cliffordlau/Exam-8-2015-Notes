...
g <-
  ggplot(d1, aes(`Annual Claim Total ($)`, `# of Policies`)) +
  stat_ecdf() + coord_flip(xlim=c(0, max(d1[1]))) + 
  ylab("Cumulative % of Policies") + xlab("Losses") + theme_minimal() +
  geom_vline(aes(xintercept = minlr * sp), linetype = "longdash") +
  geom_vline(aes(xintercept = maxlr * sp), linetype = "longdash") +
  scale_y_continuous(labels = percent) + 
  scale_x_continuous(breaks = sort(c(d1[[1]],  minlr*sp,maxlr*sp)), labels = dollar) +
