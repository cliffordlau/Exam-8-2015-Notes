...
* Same ELFs $\forall$ **class** $\in$ a given **HG** + **State** + **Limit**
    * HGs are defined countrywide $\because$ injuries mix $\in$ class $\approx$
...
Varies by **Limit** and **Injury Type** and either:

* Current **hazard groups** $j$ at the **State** level: $S_i \Big( \dfrac{L}{\mu_{i,j}} \Big)$ ***OR***;
* Individual **Classes** at the **Countrywide** level: $S_i \Big( \dfrac{L}{\mu_{i,c}} \Big)$
...
$R_j(L) = \sum\limits_i w_{i, j} S_i{\dfrac{L}{\mu_{i, j}}}$

$R_c(L) = \sum\limits_i w_{i, c} S_i{\dfrac{L}{\mu_{i, c}}}$
...
Basically just credibility weighting the individual elements
...
From 4. we have credibility weighted XS ratio vectors exist for each class; Now group similar vectors into potential HGs
...
Advantage: Minimizes the relative error in estimating excess premium

* However, analysis not sensitive to distance measure $\Rightarrow$ use traditional euclidean distance
...
$x_i =$ sample of r.v. with mean $\bar{x}$ and $\sigma \: s$

NCCI didn't standardize before doing the cluster:  
<span style="color:red">Know</span>
...
***1. Pick the number of cluster k*** <span style="color:red">Important</span>
...
***2. Start with a random initial assignment of classes to the k clusters***

***3. Compute the centroid of each clusters $i$ for the k clusters***

$\overline{R}_i = \dfrac{\sum_{c \in HG_i} w_c \vec{R}_c}{\sum_{c \in HG_i} w_c}$

* Premium weighted centroid XS ratio vector
* $w_c =$ % of total premium across all classes in all HGs
* Prevent small classes from having too much influences

***4. For each class, find the closest centroid based on L^2^ distance $\Rightarrow$ Assign the class to that cluster***
